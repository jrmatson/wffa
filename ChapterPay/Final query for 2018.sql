...
/* Each total is subtracted by 3 to account for leaders that have been added to each chapter as an additionals to support their work with */
/* the chapters. e.g. ExecDir, Exec Admin, Political, and the association president */
...
/* for the sake of chapter pay. Life members are only counted in their first year as a life member because they only pay their membership fee once and its in 
/* that first year. The window for new (first time) life memberships is June 1 of the previous membership year to May 31 of the current membership year */
SELECT local_chapter_40 Chapter, member_type_41 Member_type, count(entity_id) Count
